...
BEGIN
    ...
    INSERT INTO @extschema@.sessions VALUES (v_id, p_username, txid_current());
...
BEGIN
    ...
    SELECT user_name, created INTO v_username, v_created
      FROM @extschema@.sessions WHERE transaction_id = txid_current();
...
-- used to delete username from the sessions table
CREATE OR REPLACE FUNCTION delete_username() RETURNS UUID AS $$
DECLARE
    v_id        UUID;
    v_created   TIMESTAMP;
...
BEGIN
    ...
    SELECT id, created INTO v_id, v_created
      FROM @extschema@.sessions WHERE transaction_id = txid_current();
    ...
    DELETE FROM @extschema@.sessions WHERE id = v_id;
    RETURN v_id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER VOLATILE;

-- everyone can run the function (that's why we have the signature)
GRANT ALL ON FUNCTION delete_username() TO PUBLIC;
